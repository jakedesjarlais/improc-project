...
class Encoder(object):
    def __init__(self, encoding_str=None):
        self.long_str = encoding_str
    ...
    def _get_tree_root(self):
        ...
        return _build_tree( [HuffmanNode(ch=ch, fq=int(fq)) for ch, fq in d.iteritems()] )
    ...
    def encode(self):
        if self._long_str:
            return marshal.dumps((cPickle.dumps(self.root), self.code_length, self.array_codes))
        else:
